...
class PorterProvider:
    ...
    def get_quote(
        ...
        if not self.is_sandbox:
            try:
                ...
                res = requests.post(f"{PORTER_BASE_URL}/get_quote", json=payload, headers=headers, timeout=5)
                ...
                print(f"Porter API quote notice: {e}")

        # Sandbox contract-accurate response
        return {
    ...
    def create_order(
        ...
        if not self.is_sandbox:
            try:
                ...
                res = requests.post(f"{PORTER_BASE_URL}/orders/create", json=payload, headers=headers, timeout=5)
                ...
                print(f"Porter API order notice: {e}")

        # Sandbox contract-accurate order creation response
        return {
    ...
    def track_order(self, order_id: str) -> Dict[str, Any]:
        ...
        if not self.is_sandbox:
            try:
                ...
                res = requests.get(f"{PORTER_BASE_URL}/orders/{order_id}", headers=headers, timeout=5)
                if res.status_code == 200:
                    data = res.json()
                    return {
                        "order_id": order_id,
                ...
                print(f"Porter API track notice: {e}")
    ...
    def cancel_order(self, order_id: str, reason: str = "Transfer updated") -> Dict[str, Any]:
        ...
        if not self.is_sandbox:
            try:
                headers = {"Authorization": self.api_key, "Content-Type": "application/json"}
                requests.post(f"{PORTER_BASE_URL}/orders/{order_id}/cancel", json={"reason": reason}, headers=headers, timeout=5)
            except Exception as e:
                print(f"Porter API cancel notice: {e}")
