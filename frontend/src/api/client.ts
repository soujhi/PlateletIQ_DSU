 ...
 * Where the API lives.
 *
 * Set VITE_API_BASE_URL explicitly in production. For two-laptop testing on a
 * LAN we default to the host the page was served from, so opening the UI at
 * http://192.168.1.42:5173 talks to the API at http://192.168.1.42:8000
 * without either laptop needing its own build.
 */
